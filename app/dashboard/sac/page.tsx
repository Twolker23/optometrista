...
export default function SacPage() {
  ...
  // SAC vê: Agendados, Não Aptos, Vendas, Renegociação e Repescagem
  const queues = [
    ...
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
        <div className="min-w-0">
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900 truncate">Gestão de Clientes - SAC</h2>
          <p className="text-sm sm:text-base text-gray-600">Acesso global - todas as unidades</p>
        </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <div className="overflow-x-auto -mx-3 sm:mx-0 px-3 sm:px-0 scrollbar-hide">
          <TabsList className="inline-flex sm:grid sm:w-full sm:grid-cols-5 min-w-max sm:min-w-0">
            {queues.map((queue) => (
              <TabsTrigger 
                key={queue.status} 
                value={queue.status}
